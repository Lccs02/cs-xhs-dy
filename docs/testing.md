...
- `npm run web:build`：Vite 生产构建通过。
- `tests/unit/web-import.test.ts`：覆盖平台识别、批量解析、危险域名拒绝、幂等更新、分类、摘录和对象关联。
- Playwright CLI 生产预览实测：首次进入 → 填写合成收藏 → 本地保存 → 自动分类 → 摘录 → 对象对照通过。
- 1440×900 桌面与 390×844 窄屏均完成截图检查；控制台 0 错误、0 警告。
- 网络检查只观察到本地预览站点静态资源，没有第三方动态请求。
